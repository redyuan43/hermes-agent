...
def _needs_server_members_intent(
    ...
    """Return True when SIYUAN must request Discord's Server Members intent.
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the SIYUAN gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents SIYUAN requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage SIYUAN response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs SIYUAN work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit SIYUAN/gateway outage notices."""
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields SIYUAN manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit SIYUAN pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your SIYUAN session")
        ...
        @tree.command(name="status", description="Show SIYUAN session status")
        ...
        @tree.command(name="stop", description="Stop the running SIYUAN agent")
        ...
        @tree.command(name="update", description="Update SIYUAN to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the SIYUAN gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a SIYUAN session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to SIYUAN in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a SIYUAN skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new SIYUAN session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by SIYUAN: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        SIYUAN has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "SIYUAN"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by SIYUAN: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="SIYUAN semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 SIYUAN handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want SIYUAN to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **SIYUAN needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ SIYUAN could not create a Discord thread for "
