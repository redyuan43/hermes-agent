...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable SIYUAN command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    SIYUAN accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal SIYUAN ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches SIYUAN
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._homeserver: str = (
            ...
            or _startup_env_secret("MATRIX_HOMESERVER")
        ).rstrip("/")
        self._access_token: str = config.token or _startup_env_secret(
            "MATRIX_ACCESS_TOKEN"
        )
        self._user_id: str = config.extra.get("user_id", "") or _startup_env_secret(
            "MATRIX_USER_ID"
        )
        ...
        self._device_id: str = config.extra.get("device_id", "") or _startup_env_secret(
            "MATRIX_DEVICE_ID"
        )
        ...
        self._recovery_key_configured = bool(_scoped_recovery_key())
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "SIYUAN" become "Agent".
        """
        if not body:
            return ""
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where SIYUAN delivers cron job results and notifications.")
