"""Safe SIYUAN Console command engine.

This module backs ``hermes console`` and is intentionally narrower than the
full SIYUAN CLI. It exposes a curated set of native adapters that can later be
...
class HermesConsoleEngine:
    """Curated line-command executor for SIYUAN Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "SIYUAN Console does not run shell syntax. Use one supported "
                    "SIYUAN command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show SIYUAN component status.", _status)
        self.register(("version",), "version", "Show SIYUAN version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent SIYUAN logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update SIYUAN configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for SIYUAN Console."""
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update SIYUAN configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported SIYUAN Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in SIYUAN Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in SIYUAN Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in SIYUAN Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in SIYUAN Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in SIYUAN Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in SIYUAN Console.",
            ("skills", "publish"): "`skills publish` is not available in SIYUAN Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in SIYUAN Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in SIYUAN Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in SIYUAN Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in SIYUAN Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in SIYUAN Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in SIYUAN Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in SIYUAN Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in SIYUAN Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in SIYUAN Console.",
            ("kanban", "gc"): "`kanban gc` is not available in SIYUAN Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in SIYUAN Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in SIYUAN Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in SIYUAN Console.")
...
def run_console_repl(
    *,
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("SIYUAN Console. Type `help` for commands, `exit` to quit.", file=stdout)
