...
TIPS = [
    ...
    "Ctrl+Z suspends SIYUAN to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs SIYUAN itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire SIYUAN home directory.",
    ...
    "hermes gateway install sets up SIYUAN as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call SIYUAN tools programmatically — results stay out of context.",
    ...
    "When exiting, SIYUAN prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "SIYUAN runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to SIYUAN via a local macOS server.",
    ...
    "SIYUAN loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs SIYUAN as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make SIYUAN your own.",
    ...
    "Before a gateway session reset, SIYUAN auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope SIYUAN state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — SIYUAN auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of SIYUAN config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while SIYUAN is working.',
    ...
    'hermes dashboard embeds the full SIYUAN TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hermes import restores a full SIYUAN backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes SIYUAN block commands when the tirith scanner itself errors out.',
...
# any project or none — SIYUAN is not a coding-only agent.
# ---------------------------------------------------------------------------
