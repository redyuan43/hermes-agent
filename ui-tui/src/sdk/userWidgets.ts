...
/**
 * User widget apps — SIYUAN authors its own TUI widgets, mirroring the
 ...
 *  every change, so a widget SIYUAN writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or SIYUAN) ever writes must hot-load too — a 10s poll here read
