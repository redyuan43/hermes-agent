...
Scope of the repair is deliberately narrow. SIYUAN only upgrades an npm that
...
and their other projects; SIYUAN never modifies those. When the failing npm is
one of those foreign installs, SIYUAN instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the SIYUAN-managed Node root *npm* lives in, else ``None``.
...
def upgrade_managed_npm(
    ...
    *,
    ...
    if not quiet:
        print(
            f"→ Upgrading SIYUAN-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the SIYUAN-managed Node.js tree is in use by a "
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  SIYUAN could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a SIYUAN-managed Node tree and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a SIYUAN-managed Node.js runtime "
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a SIYUAN-managed install, or
    ...
    if prefix is not None:
        # SIYUAN owns this npm — upgrade it in place. Only an npm-range
