...
_REBUILD_SPECS = {
    ...
    "kanban_notify_subs": (
        ...
        " fallback_notifier_profile TEXT, delivery_metadata TEXT, created_at INTEGER NOT NULL,"
        " last_event_id INTEGER NOT NULL DEFAULT 0,"
        " PRIMARY KEY (task_id, platform, chat_id, thread_id))",
...
def add_notify_sub(
    ...
    *,
    ...
    with write_txn(conn):
        conn.execute(
            ...
            INSERT OR IGNORE INTO kanban_notify_subs
                (task_id, platform, chat_id, chat_type, thread_id, user_id,
                 notifier_profile, fallback_platform, fallback_chat_id, fallback_thread_id,
                 fallback_user_id, fallback_notifier_profile, delivery_metadata,
        ...
        if metadata_json:
            # Duplicate subscribe from the same chat/thread should refresh
            # the routing anchor; Telegram DM-topic notifications need
            # the latest reply anchor to stay in-thread.
            conn.execute(
                """
                UPDATE kanban_notify_subs
                   SET delivery_metadata = COALESCE(?, delivery_metadata)
                 WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?
                """,
                (metadata_json, task_id, platform, chat_id, thread_id or ""),
            )
        if fallback_platform or fallback_chat_id or fallback_thread_id or fallback_user_id or fallback_notifier_profile:
            # Self-heal legacy/idempotent rows by filling missing fallback
            # values from the call; never overwrite values that are already
            # set by a previous route.
            conn.execute(
                """
                UPDATE kanban_notify_subs
                   SET fallback_platform = CASE
                ...
                (
                    ...
                    fallback_user_id, fallback_notifier_profile, task_id, platform,
