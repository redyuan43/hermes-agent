...
and injects content-free wake hints into SIYUAN' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # SIYUAN only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active SIYUAN turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    ...
    and test contexts. The flow persists ``RAFT_PROFILE`` to the SIYUAN env
    ...
    print_info("Connect SIYUAN to Raft as an external agent.")
