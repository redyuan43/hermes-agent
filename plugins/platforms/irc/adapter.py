"""
IRC Platform Adapter for SIYUAN.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the SIYUAN agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :SIYUAN")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :SIYUAN shutting down")
...
def interactive_setup() -> None:
    ...
    print_info("Connect SIYUAN to an IRC network. Uses Python stdlib — no extra packages needed.")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :SIYUAN (cron)")
