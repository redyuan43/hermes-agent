...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not $hermes){throw "SIYUAN is not installed on the remote Windows host."}',
    'Assert-NoReparse $hermes $false',
    'if($explicit -and $hermes -ne $explicit){throw "The configured SIYUAN path is not an executable file."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update SIYUAN on the remote Windows host before connecting with Desktop SSH.')
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="SIYUAN SSH"', 'powershell.exe -NoLogo')
