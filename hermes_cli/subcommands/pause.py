...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "SIYUAN paused"
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  SIYUAN resumed — dispatch picks up on the next tick.")
    else:
        print(f"SIYUAN is not paused (no sentinel at {sentinel_path()}).")
    return 0
