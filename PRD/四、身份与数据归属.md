...
- 核心原则：一个用户身份，多种入口；同机共用 General；跨机数据隔离
...
```text
nano2/general != nx2/general != nx3/general
```

设备间不会因为 Agent 名称相同而自动共享配置或数据。
...
已确认采用完整 Profile 迁移，但“完整”表示所有文件都有盘点、去向和归档，
不表示用一个数据库覆盖另一个数据库。

迁移规则：

1. 确定 nano2 Owner General 的唯一 `agent_id`，以现有 `default` Profile
   作为主 Profile。
2. 保留 `default` 的 `state.db`、现有会话、记忆、工作文件和用户数据。
3. 全量盘点 `matrix-general` 的 SOUL、AGENTS、Skills、MoA 配置、日志和
   工作文件，并生成迁移清单。
4. 将通用人格、能力和调度规则合并到 Owner General。
5. Matrix 的仅被提及时响应、Room/Thread 隔离、附件路径等通道规则留在
   Matrix Gateway 配置，不进入共享 SOUL。
6. 同名文件冲突时不直接覆盖：主 Profile 保持运行版本，旧版本进入只读
   归档并记录在迁移清单中。
7. 微信主入口和 Matrix `@general` 都映射到同一个 Owner General，但继续
   使用各自的 Chat、Room 和 Thread 会话键。
8. 完成人工验证后，将原 `matrix-general` 目录转为只读归档。
...
Agent 数据的共享范围以物理设备为边界。同一设备的两个 General 入口可以
访问同一份 Agent 数据；不同设备即使属于同一用户，也不会直接挂载或实时
同步这些数据。
...
未来由 Federation Console 管理 Skills、Agent 定义和 SOUL 的跨设备复用，
采用版本化发布和显式导入：

- 源设备发布不可变配置版本。
- 目标设备明确选择并确认导入。
- 导入后形成目标设备自己的副本，可以独立修改。
- 源设备后续变化不会自动覆盖目标设备。
- 会话、记忆、用户文件、凭据、Grant 和审批数据永远不进入发布包。

V1 只预留该概念、后台位置和状态展示，不提供实际发布、导入、自动更新或
双向同步接口。
...
12. `default/state.db` 不会被 `matrix-general/state.db` 覆盖。
13. Matrix 通道规则不会改变微信入口的响应行为。
14. 不同设备不会自动同步 Agent 配置和数据。
...
- 同一设备的微信与 Matrix `@general` 共用一个 Owner General。
- 不同设备的同名 Agent 默认完全独立。
