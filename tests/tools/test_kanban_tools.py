...
def test_create_rejects_assignee_outside_profile_allowlist(monkeypatch, worker_env, tmp_path):
    """Profiles can constrain orchestration to a named worker roster."""
    home = tmp_path / "allowlist-home" / ".hermes"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  allowed_assignees: [family, work, work2]\n"
    )
    monkeypatch.setenv("HERMES_HOME", str(home))

    from tools import kanban_tools as kt
    out = kt._handle_create({"title": "blocked", "assignee": "other"})
    assert "not allowed" in out
    assert _list_subs_for_task("missing") == []


def test_create_uses_configured_completion_delivery(monkeypatch, worker_env, tmp_path):
    """A configured completion target replaces the origin-chat subscription."""
    home = tmp_path / "delivery-home" / ".hermes"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n"
        "  completion_delivery:\n"
        ...
        "    platform: matrix\n"
        "    chat_id: '!delivery:example.test'\n"
    )
    monkeypatch.setenv("HERMES_HOME", str(home))
    monkeypatch.setenv("MATRIX_ACCESS_TOKEN", "test-token")
    monkeypatch.setenv("MATRIX_HOMESERVER", "https://matrix.example.test")
    monkeypatch.setenv("MATRIX_USER_ID", "@bot:example.test")
    ...
    assert subs[0]["chat_id"] == "!delivery:example.test"
...
def test_create_delivery_override_beats_profile_default(monkeypatch, worker_env, tmp_path):
    home = tmp_path / "override-home" / ".hermes"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n"
        "  completion_delivery:\n"
    ...
    out = kt._handle_create({
        ...
        "delivery": {
            "sender_profile": "default",
            "platform": "weixin",
            "chat_id": "wxid_override",
        },
    })
    d = json.loads(out)
    assert d.get("ok") is True, d
    sub = _list_subs_for_task(d["task_id"])[0]
    assert sub["platform"] == "weixin"
    assert sub["chat_id"] == "wxid_override"


def test_create_rejects_invalid_delivery_before_task_write(monkeypatch, worker_env):
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt
    ...
    out = kt._handle_create({
        ...
        "delivery": {
            ...
            "use_home_channel": True,
        },
    })
    assert "invalid delivery" in json.loads(out)["error"]
    conn = kb.connect()
    try:
...
def test_delivery_home_channel_uses_sender_profile_scope(
    monkeypatch, worker_env, tmp_path,
):
    profile_home = tmp_path / ".hermes" / "profiles" / "sender"
    profile_home.mkdir(parents=True)
    (profile_home / "config.yaml").write_text("{}\n")
    (profile_home / ".env").write_text(
        "MATRIX_ACCESS_TOKEN=test-token\n"
        "MATRIX_HOMESERVER=https://matrix.example.test\n"
        "MATRIX_HOME_ROOM=!sender-home:example.test\n"
    )
    monkeypatch.setenv("MATRIX_HOME_ROOM", "!caller-home:example.test")

    from tools import kanban_tools as kt
    out = kt._handle_create({
        "title": "profile scoped home",
        "assignee": "peer",
        "delivery": {
    ...
    assert d.get("ok") is True, d
    sub = _list_subs_for_task(d["task_id"])[0]
    assert sub["chat_id"] == "!sender-home:example.test"
    assert sub["notifier_profile"] == "sender"
