...
manual BotFather token copy-paste. SIYUAN talks only to the Nous onboarding
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting SIYUAN Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the SIYUAN Telegram onboarding service.")
