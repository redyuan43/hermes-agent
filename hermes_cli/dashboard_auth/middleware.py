...
def _mobile_access_token(request: Request) -> str:
    ...
    Native clients cannot safely depend on browser cookies because their WebView
    origin differs from the remote Hermes host. Keep this separate from the
    generic ``Authorization: Bearer`` token-auth seam: those are explicitly
    service credentials and are only valid on registered token routes.
    """
