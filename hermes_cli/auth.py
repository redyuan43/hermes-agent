...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize SIYUAN:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"SIYUAN is waiting for the OAuth callback on {redirect_uri}")
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "SIYUAN is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("SIYUAN is not logged into Nous Portal.",
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in SIYUAN auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("SIYUAN will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("SIYUAN will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(SIYUAN creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in SIYUAN auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(SIYUAN creates its own local OAuth session)")
    print()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting SIYUAN login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting SIYUAN login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("SIYUAN will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hermes model` or configure an API key to use SIYUAN.")
        else:
