        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # pre-filter to skip claiming events for subscriptions nobody
                    # can possibly deliver — the precise per-profile check
                    # (via gateway/authz_mixin.py::_authorization_adapter) still
                    # runs at delivery time below and rewinds the claim if it
                    # resolves to None.
                    # Without this, a subscription owned by a secondary profile
                    # on a platform the DEFAULT profile never connected (for
                    # example, beta owns discord and default doesn't) was dropped
                    # before ever being claimed, so it silently never retried.
