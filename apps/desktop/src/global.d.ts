...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // SIYUAN Cloud: one portal login powers discovery + silent per-agent
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a SIYUAN Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted SIYUAN Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected SIYUAN Cloud org (slug or id) to
...
// A discovered SIYUAN Cloud agent — the trimmed DTO from NAS GET /api/agents.
