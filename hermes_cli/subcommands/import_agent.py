...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into SIYUAN",
        description=(
            "One-command import of another coding agent's setup into SIYUAN. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their SIYUAN equivalents. "
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing SIYUAN items on name conflicts (default: skip)",
    )
    parser.add_argument(
