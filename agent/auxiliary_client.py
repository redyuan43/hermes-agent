...
def _client_cache_key(
    ...
    *,
    ...
    # nano2 local: keep `task` in the cache key for claude-code-cli too —
    # a cached reference client must not bypass the resolver guard.
    task_key = (
        (task or "", _task_prefers_fast_model(task))
        if provider in {"auto", "claude-code-cli"}
        else ""
    )
