"""
Photon Spectrum (iMessage) platform adapter for SIYUAN.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a SIYUAN dep
...
# artifacts so SIYUAN sees the link once, not a follow-up "(attachment)" prompt.
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so SIYUAN does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use SIYUAN defaults).
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any SIYUAN checkout's sidecar entry point.
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the SIYUAN gateway (which spawns the sidecar and "
...
def register(ctx) -> None:
    """Called by the SIYUAN plugin loader at startup."""
